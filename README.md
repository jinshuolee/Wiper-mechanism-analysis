...
在雨刷機構中，作用於連桿機構的外力主要來自於雨刷片在擋風玻璃上滑動所產生的摩擦力。

然而，雨刷的摩擦特性非常複雜，因為它必須考慮不同的負載條件和環境因素。因此，在本研究中，我們將摩擦力簡化為作用於雨刷片連桿上的輸入扭力。

在工作過程中，雨刷臂不可避免地會因摩擦力的作用而變形。這種變形與所施加的力之間的關係可以用一個二階微分方程式來描述，該方程式相當複雜，難以用簡單的計算方法求解。因此，我們對變形關係做出了一些簡化假設，從而可以使用六連桿機構模型來計算。
